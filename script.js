...
if (zoomSlider) {
  zoomSlider.addEventListener("input", () => {
    const val = zoomSlider.value / 120;
...
//// ======= AUDIO SETUP (mobile-friendly) =======
async function ensureMic() {
  if (micStream) return;

  // Quick platform hint (useful for messaging)
  const isAndroidWebView = !!(window.Capacitor && (typeof window.Capacitor.getPlatform === 'function' ? window.Capacitor.getPlatform() === 'android' : false))
                         || (!!window.cordova && /Android/i.test(navigator.userAgent))
                         || /Android/.test(navigator.userAgent) && /wv/.test(navigator.userAgent); // approximate

  // Check for getUserMedia
  if (!navigator.mediaDevices?.getUserMedia) {
    showMsg('❌ Microphone not supported on this platform');
    throw new Error('getUserMedia not supported');
  }

  // If browser supports Permissions API, query microphone permission state (helpful on mobile)
  try {
    if (navigator.permissions && typeof navigator.permissions.query === 'function') {
      const p = await navigator.permissions.query({ name: 'microphone' });
      if (p.state === 'denied') {
        // Permission already denied — instruct user how to enable
        showMsg('❌ Microphone permission blocked. Open app settings and allow Microphone, then reopen the app.');
        throw new Error('permission-denied');
      }
      // keep listening for changes (optional)
      p.onchange = () => {
        if (p.state === 'granted') hideMsg();
      };
    }
  } catch (e) {
    // ignore permission query errors (some WebViews don't implement it)
    console.debug('permissions query not available or failed', e);
  }

  // Try to get mic stream
  try {
    // mobile-friendly audio constraints: request raw mic (no automatic processing)
    ...
    // More helpful messages depending on error
    console.error('getUserMedia error:', err);
    if (err && (err.name === 'NotAllowedError' || err.name === 'SecurityError' || err.name === 'PermissionDeniedError')) {
      if (isAndroidWebView) {
        showMsg('❌ Microphone access denied. Make sure app permission for Microphone is enabled (Settings → Apps → YourApp → Permissions).');
      } else {
        showMsg('❌ Microphone access denied. Please allow microphone in the browser prompt.');
      }
    } else {
      showMsg('❌ Unable to open microphone: ' + (err.message || err.name || 'unknown'));
    }
    throw err;
  }

  // Resume AudioContext (await - important on mobile where resume may be async)
  try { await audioCtx.resume(); } catch(e){ console.warn('audioCtx.resume failed', e); }

  // Create mic source safely
  try {
    micSource = audioCtx.createMediaStreamSource(micStream);
  } catch(e){
    showMsg('❌ Failed creating MediaStreamSource');
    console.error(e);
    throw e;
  }
  ...
  try {
    ...
    reverbPreDelay.delayTime.value = (typeof reverbPreDelayMs !== 'undefined' ? reverbPreDelayMs/1000 : 0.02);
    ...
    // connect if objects exist
    try { micSource.connect(reverbPreDelay); reverbPreDelay.connect(convolver); convolver.connect(reverbWet); reverbWet.connect(fxSumGain); } catch(e){ console.warn('reverb wiring failed', e); }
  ...
  try {
    ...
    try { micSource.connect(delayNode); delayNode.connect(delayWet); delayWet.connect(fxSumGain); } catch(e){ console.warn('delay wiring failed', e); }
  } catch(e){ console.warn('delay init failed', e); }
  ...
  try {
    ...
    try { micSource.connect(flangerDelay); } catch(e){ console.warn('flanger mic connect failed', e); }
    try { flangerLFO.start(); } catch(e){ /* ignore if already started or not allowed */ }
  } catch(e){ console.warn('flanger init failed', e); }

  // EQ (created when toggled on)
  eq = null;

  // Dry path
  try { micSource.connect(dryGain); } catch(e){ console.warn('dry path connect failed', e); }
  ...
  } catch(e){ console.warn('mixDest/processedStream failed', e); }

  // Live monitor
  try {
    liveMicMonitorGain = audioCtx.createGain(); liveMicMonitorGain.gain.value = liveMicMonitoring ? 1 : 0;
    dryGain.connect(liveMicMonitorGain);
    fxSumGain.connect(liveMicMonitorGain);
    liveMicMonitorGain.connect(audioCtx.destination);
  } catch(e){ console.warn('live monitor wiring failed', e); }

  // UI updates
  hideMsg();

  // hide mic test button (if present)
  const micBtn = document.getElementById('micTestBtn');
  if (micBtn) micBtn.classList.add('hidden-by-js');

  // ensure monitor text matches state
  const monBtn = document.getElementById('monitorBtn');
  if (monBtn) monBtn.textContent = liveMicMonitoring ? 'Live MIC ON 🎤' : 'Live MIC OFF';

  console.log('Microphone initialized. processedStream ready:', !!processedStream);
}

// ======= TOGGLE EQ (mobile-safe) =======
function toggleEQ(enable){
  ...
  if (enable && !eq){
    try {
      ...
      // Connect EQ outputs into the rest of the graph only if those nodes exist
      try { if (typeof dryGain !== 'undefined') eq.high.connect(dryGain); } catch(e){ console.warn('connect eq->dry failed', e); }
      try { if (typeof delayNode !== 'undefined') eq.high.connect(delayNode); } catch(e){ /* ignore */ }
      try { if (typeof reverbPreDelay !== 'undefined') eq.high.connect(reverbPreDelay); } catch(e){ /* ignore */ }
      try { if (typeof flangerDelay !== 'undefined') eq.high.connect(flangerDelay); } catch(e){ /* ignore */ }
    } catch (e) {
      console.error('toggleEQ enable failed', e);
    ...
    try{ eq.low.disconnect(); eq.mid.disconnect(); eq.high.disconnect(); }catch{}
    try{ micSource.disconnect(); }catch{}
    try { micSource.connect(dryGain); } catch(e){ console.warn('mic->dry reconnect failed', e); }
    try { if (typeof delayNode !== 'undefined') micSource.connect(delayNode); } catch(e){}
    try { if (typeof reverbPreDelay !== 'undefined') micSource.connect(reverbPreDelay); } catch(e){}
    try { if (typeof flangerDelay !== 'undefined') micSource.connect(flangerDelay); } catch(e){}
    eq = null;
  }
}
...
function updateDelayFromTempo(){
  ...
  if (typeof delayNode === 'undefined' || !delayNode) return; // guard
